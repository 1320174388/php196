@extends('layouts.admin')
@section('title','添加角色页')
@section('content')

		<div class="finance-content-middle">
			<div class="finance-content-middle-form finance-add-agent-ad">
				<h3>添加角色</h3>
				<div class="finance-add-agent">
                    @if (count($errors) > 0)
                        <div class="alert alert-danger" >
                            <ul style="margin-left: 185px ">
                                @if(is_object($errors))
                                    @foreach ($errors->all() as $error)
                                        <li style="color:red">{{ $error }}</li>
                                    @endforeach
                                @endif
                            </ul>
                        </div>
                    @endif
					<form action="{{ url('/admin/role') }}" enctype="multipart/form-data" method="post">
						{{csrf_field()}}
						<p>
							<label>角色：</label>
							<input type="text" class="form-control" name="role_name" value="{{ old('role_id') }}" placeholder="请输入角色">
							<em>*(必填)</em>
						</p>
						<p>
							<label>描述：</label>
							<input type="text" value="{{ old('permission_id') }}" class="form-control" name="role_description" placeholder="请输入描述">
							<em>*(必填)</em>
						</p>
						<p><div class="form-group">
							<input type="submit"  style="width: 160px; height: 40px; background: #1e82d4; color:white; margin-left: 70px;" value="添加">
	                        <input type="button" class="back" style="width: 160px; height: 40px; background: #1e82d4; color:white; margin-left: 30px;" onclick="history.go(-1)" value="返回">
	                    </div></p>
					</form>
				</div>
			</div>
		</div>
@stop

